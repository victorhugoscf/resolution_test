...
<div class="container mt-5">
  <h1 class="fw-bold text-primary mb-4">📝 {{ $sale->exists ? 'Editar Venda' : 'Nova Venda' }}</h1>
  @if ($errors->any())
  ...
  @endif
  <form action="{{ $sale->exists ? route('sales.update', $sale->id) : route('sales.store') }}" method="POST" class="needs-validation" novalidate>
    @csrf
    @if($sale->exists)
      @method('PUT')
    @endif
          ...
          <div class="payment-row mb-3 p-3 border rounded bg-light">
            <div class="row g-3 align-items-center">
              ...
              @if($payment->method == 'credit')
                <div class="col-md-2 installment-container">
                  <select name="installment_sales[{{ $index }}][installment_number]" class="form-select installment-select">
                    <option value="1" {{ old('payments.' . $index . '.installments', $sale->installmentSales->count() ?: 1) == 1 ? 'selected' : '' }}>1x (À vista)</option>
                    @for ($i = 2; $i <= 12; $i++)
                      <option value="{{ $i }}" {{ old('payments.' . $index . '.installments', $sale->installmentSales->count() ?: 1) == $i ? 'selected' : '' }}>{{ $i }}x</option>
                    @endfor
                  </select>
                </div>
                <div class="col-md-4 d-flex justify-content-end align-items-center">
                  <span class="installment-value text-muted me-2">
                    Parcela: R$ {{ number_format($payment->total_amount / ($sale->installmentSales->count() ?: 1), 2, ',', '.') }}
                  </span>
              ...
              @else
                <div class="col-md-2 installment-container" style="display: none;">
                  <select name="payments[{{ $index }}][installments]" class="form-select installment-select" disabled>
